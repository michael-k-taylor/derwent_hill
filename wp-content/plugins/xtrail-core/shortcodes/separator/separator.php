<?php
namespace XtrailCore\CPT\Shortcodes\Separator;
...
class Separator implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                    => esc_html__( 'Separator', 'xtrail-core' ),
					'base'                    => $this->base,
					...
					'params'                  => array(
						...
						array(
							...
							'value'      => array(
								...
								esc_html__( 'Full Width', 'xtrail-core' ) => 'full-width'
							)
						),
						array(
							'type'       => 'dropdown',
						...
						array(
							...
							'heading'     => esc_html__( 'Style', 'xtrail-core' ),
							'value'       => array(
								esc_html__( 'Default', 'xtrail-core' ) => '',
								esc_html__( 'Dashed', 'xtrail-core' )  => 'dashed',
								esc_html__( 'Solid', 'xtrail-core' )   => 'solid',
								esc_html__( 'Dotted', 'xtrail-core' )  => 'dotted'
							),
							'save_always' => true
						),
						array(
	...
	public function render( $atts, $content = null ) {
		...
		$html = xtrail_core_get_shortcode_module_template_part( 'templates/separator-template', 'separator', '', $params );
	...
	private function getHolderStyles( $params ) {
		...
		if ( $params['width'] !== '' ) {
			if ( xtrail_select_string_ends_with( $params['width'], '%' ) || xtrail_select_string_ends_with( $params['width'], 'px' ) ) {
				$styles[] = 'width: ' . $params['width'];
			} else {
				$styles[] = 'width: ' . xtrail_select_filter_px( $params['width'] ) . 'px';
			}
		}
		
		if ( $params['thickness'] !== '' ) {
			$styles[] = 'border-bottom-width: ' . xtrail_select_filter_px( $params['thickness'] ) . 'px';
		}
		
		if ( $params['top_margin'] !== '' ) {
			if ( xtrail_select_string_ends_with( $params['top_margin'], '%' ) || xtrail_select_string_ends_with( $params['top_margin'], 'px' ) ) {
				$styles[] = 'margin-top: ' . $params['top_margin'];
			} else {
				$styles[] = 'margin-top: ' . xtrail_select_filter_px( $params['top_margin'] ) . 'px';
			}
		}
		
		if ( $params['bottom_margin'] !== '' ) {
			if ( xtrail_select_string_ends_with( $params['bottom_margin'], '%' ) || xtrail_select_string_ends_with( $params['bottom_margin'], 'px' ) ) {
				$styles[] = 'margin-bottom: ' . $params['bottom_margin'];
			} else {
				$styles[] = 'margin-bottom: ' . xtrail_select_filter_px( $params['bottom_margin'] ) . 'px';
			}
		}
