...
if ( ! function_exists( 'xtrail_select_add_linear_icon_pack' ) ) {
	function xtrail_select_add_linear_icon_pack( $iconCollections ) {
		$iconCollections['linear_icons'] = new XtrailSelectClassLinearIcons( 'Linear Icons', 'linear_icon' );
		
		return $iconCollections;
	}
	
	add_filter( 'xtrail_select_filter_add_icon_pack_into_collection', 'xtrail_select_add_linear_icon_pack' );
}

if ( ! function_exists( 'xtrail_select_add_linear_icon_pack_option' ) ) {
	function xtrail_select_add_linear_icon_pack_option( $options ) {
		$options['linear_icons'] = esc_html__( 'Linear Icons', 'xtrail' );
		
		return $options;
	}
	
	add_filter( 'xtrail_select_filter_add_icon_pack_into_options', 'xtrail_select_add_linear_icon_pack_option' );
}

class XtrailSelectClassLinearIcons implements iXtrailSelectInterfaceIconCollection {
	public $icons;
	public $title;
	public $param;
	public $styleUrl;
	...
	function __construct($title = "", $param = "") {
		...
		$this->param = $param;
		$this->setIconsArray();
		$this->styleUrl = SELECT_FRAMEWORK_ICONS_ROOT . '/linear-icons/style.css';
	}
	...
	public function render($icon, $params = array()) {
		...
		$html .= '<span aria-hidden="true" class="qodef-icon-linear-icons lnr ' . $icon . ' ' . $iconClass . '" ' . $iconAttributesString . '></span>';
