...
foreach ( glob( SELECT_FRAMEWORK_ICONS_ROOT_DIR . '/*/load.php' ) as $module_load ) {
	if ( xtrail_select_is_customizer_item_enabled( $module_load, 'xtrail_performance_disable_icon_pack_' ) ) {
		include_once $module_load;
	}
}

if ( ! function_exists( 'xtrail_select_get_icon_pack_collections' ) ) {
	function xtrail_select_get_icon_pack_collections() {
		$options = apply_filters( 'xtrail_select_filter_add_icon_pack_into_collection', $options = array() );
		
		return $options;
	}
}

if ( ! function_exists( 'xtrail_select_get_icon_pack_options' ) ) {
	function xtrail_select_get_icon_pack_options() {
		$options = apply_filters( 'xtrail_select_filter_add_icon_pack_into_options', $options = array() );
		
		return $options;
	}
}

if ( ! function_exists( 'xtrail_select_get_icon_pack_social_options' ) ) {
	function xtrail_select_get_icon_pack_social_options() {
		$options = apply_filters( 'xtrail_select_filter_add_icon_pack_into_social_options', $options = array() );
...
/*
 * Class: XtrailSelectClassIconCollections
 */
class XtrailSelectClassIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = xtrail_select_get_icon_pack_collections();
		$this->iconPackOptions       = xtrail_select_get_icon_pack_options();
		$this->iconPackSocialOptions = xtrail_select_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = xtrail_select_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'xtrail' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'heading'     => esc_html__( 'Icon', 'xtrail' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'xtrail' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'xtrail' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'xtrail' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'xtrail' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'xtrail' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'xtrail' )       => 'fa-lg',
			esc_html__( 'Small', 'xtrail' )      => 'fa-2x',
			esc_html__( 'Medium', 'xtrail' )     => 'fa-3x',
			esc_html__( 'Large', 'xtrail' )      => 'fa-4x',
			esc_html__( 'Very Large', 'xtrail' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iXtrailSelectInterfaceIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'xtrail' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'xtrail' ) ] = '';
	...
	public function getDropdownCartIcon( $iconPack, $return ) {
		if ( $this->hasIconCollection( $iconPack ) ) {
			$iconsObject = $this->getIconCollection( $iconPack );
			$iconHTML    = $iconsObject->getDropdownCartIcon();
			
			if ( $return ) {
				return wp_kses_post( $iconHTML );
			} else {
				echo wp_kses_post( $iconHTML );
			}
		}
	...
	public function getMenuIcon( $iconPack, $return ) {
		if ( $this->hasIconCollection( $iconPack ) ) {
			$iconsObject = $this->getIconCollection( $iconPack );
	...
	public function getMenuCloseIcon( $iconPack, $return ) {
		if ( $this->hasIconCollection( $iconPack ) ) {
			$iconsObject = $this->getIconCollection( $iconPack );
