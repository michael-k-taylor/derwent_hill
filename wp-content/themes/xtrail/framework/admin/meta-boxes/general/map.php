...
if ( ! function_exists( 'xtrail_select_map_general_meta' ) ) {
	function xtrail_select_map_general_meta() {
		
		$general_meta_box = xtrail_select_create_meta_box(
			array(
				'scope' => apply_filters( 'xtrail_select_filter_set_scope_for_meta_boxes', array( 'page', 'post' ), 'general_meta' ),
		...
		xtrail_select_create_meta_box_field(
			array(
				...
				'type'        => 'text',
				'label'       => esc_html__( 'Slider Shortcode', 'xtrail' ),
				'description' => esc_html__( 'Paste your slider shortcode here', 'xtrail' ),
		...
		xtrail_select_create_meta_box_field(
			array(
				...
				'type'          => 'yesno',
				'default_value' => 'no',
				'label'         => esc_html__( 'Always put content behind header', 'xtrail' ),
				'description'   => esc_html__( 'Enabling this option will put page content behind page header', 'xtrail' ),
				'parent'        => $general_meta_box
			)
		);
		
		$qodef_content_padding_group = xtrail_select_add_admin_group(
			array(
				'name'        => 'content_padding_group',
				'title'       => esc_html__( 'Content Styles', 'xtrail' ),
				'description' => esc_html__( 'Define styles for Content area', 'xtrail' ),
				'parent'      => $general_meta_box
			)
		);
		
			$qodef_content_padding_row = xtrail_select_add_admin_row(
				array(
			...
			$qodef_content_padding_row_1 = xtrail_select_add_admin_row(
				array(
				...
				xtrail_select_create_meta_box_field(
					array(
						...
						'type'   => 'textsimple',
						'label'  => esc_html__( 'Content Padding (eg. 10px 5px 10px 5px)', 'xtrail' ),
				...
				xtrail_select_create_meta_box_field(
					array(
						...
						'type'    => 'textsimple',
						'label'   => esc_html__( 'Content Padding for mobile (eg. 10px 5px 10px 5px)', 'xtrail' ),
						'parent'  => $qodef_content_padding_row_1,
						'args'        => array(
							'col_width' => 4
						)
		...
		xtrail_select_create_meta_box_field(
			array(
				...
				'options'       => array(
					...
					'qodef-grid-1300' => esc_html__( '1300px', 'xtrail' ),
		...
		xtrail_select_create_meta_box_field(
			array(
				...
				'description' => esc_html__( 'Choose a space between content layout and sidebar layout for your page', 'xtrail' ),
		...
		xtrail_select_create_meta_box_field(
			...
			$boxed_container_meta = xtrail_select_add_admin_container(
				array(
					'parent'          => $general_meta_box,
		...
		xtrail_select_create_meta_box_field(
			...
			$paspartu_container_meta = xtrail_select_add_admin_container(
				array(
					'parent'          => $general_meta_box,
		...
		xtrail_select_create_meta_box_field(
			...
			$page_transitions_container_meta = xtrail_select_add_admin_container(
				array(
					'parent'     => $general_meta_box,
				...
				$page_transition_preloader_container_meta = xtrail_select_add_admin_container(
					array(
						'parent'     => $page_transitions_container_meta,
					...
					$group_pt_spinner_animation_meta = xtrail_select_add_admin_group(
						array(
					...
					$row_pt_spinner_animation_meta = xtrail_select_add_admin_row(
						array(
					...
					xtrail_select_create_meta_box_field(
						array(
							'type'         => 'image',
							'name'          => 'qodef_smooth_pt_spinner_image_meta',
							'label'         => esc_html__( 'Preloader Image', 'xtrail' ),
							'parent'        => $row_pt_spinner_animation_meta,
							'dependency' => array(
								'show' => array(
									'qodef_smooth_pt_spinner_type_meta' => 'xtrail_spinner'
								)
							)
						)
					);
		...
		xtrail_select_create_meta_box_field(
			array(
				...
				'type'        => 'select',
				'label'       => esc_html__( 'Show Comments', 'xtrail' ),
				'description' => esc_html__( 'Enabling this option will show comments on your page', 'xtrail' ),
				'parent'      => $general_meta_box,
				'options'     => xtrail_select_get_yes_no_select_array()
			)
		);
	...
	add_action( 'xtrail_select_action_meta_boxes_map', 'xtrail_select_map_general_meta', 10 );
}

if ( ! function_exists( 'xtrail_select_container_background_style' ) ) {
	/**
	 * Function that return container style
	 */
	function xtrail_select_container_background_style( $style ) {
		...
		$current_style = xtrail_select_dynamic_css( $container_selector, $container_class );
		$current_style = $current_style . $style;
		
		return $current_style;
	}
	
	add_filter( 'xtrail_select_filter_add_page_custom_style', 'xtrail_select_container_background_style' );
}