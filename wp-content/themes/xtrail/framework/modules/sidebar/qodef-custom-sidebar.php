 ...
 */
if ( ! class_exists( 'XtrailSelectClassSidebar' ) ) {
	class XtrailSelectClassSidebar {
		var $sidebars = array();
		var $stored = "";
		...
		function __construct() {
			...
			$this->title  = esc_html__( 'Custom Widget Area', 'xtrail' );
		...
		function load_assets() {
			...
			wp_enqueue_script( 'qodef-admin-sidebar-script', SELECT_ROOT . '/framework/admin/assets/js/qodef-ui/qodef-sidebar.js' );
			wp_enqueue_style( 'qodef-admin-sidebar', SELECT_ROOT . '/framework/admin/assets/css/qodef-sidebar.css' );
		}
		
		//widget form template
		function template_add_widget_field() {
			$nonce = '<input type="hidden" name="qodef-delete-sidebar" value="' . wp_create_nonce('qodef-delete-sidebar') . '" />';
			
			echo "\n<script type='text/html' id='qodef-add-widget'>";
			echo "\n  <form class='qodef-add-widget' method='POST'>";
			echo "\n  <h3>" . esc_html($this->title) . "</h3>";
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '" . esc_attr__('Enter Name of the new Widget Area', 'xtrail') . "' name='qodef-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='" . esc_attr__('Add Widget Area', 'xtrail') . "' />";
			echo "\n    " . $nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function add_sidebar_area() {
			if ( ! empty( $_POST['qodef-add-widget'] ) ) {
				...
				$name           = $this->get_name( $_POST['qodef-add-widget'] );
		...
		function register_custom_sidebars() {
			...
			$args = apply_filters( 'xtrail_select_filter_custom_widget_args', $args );
