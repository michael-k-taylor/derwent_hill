...
if (!class_exists('XtrailSelectClassMobileNavigationWalker')) {
	class XtrailSelectClassMobileNavigationWalker extends Walker_Nav_Menu {
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if(esc_attr( $item->url ) === '#') {
				$current_a .= ' qodef-mobile-no-link';
			}
			...
			if($item->hide == ""){
				...
				if($args->has_children) {
					$item_output .= '<span class="mobile_arrow"><i class="qodef-sub-arrow ion-chevron-right"></i></span>';
				}
			}
