<?php
namespace XtrailCore\CPT\Shortcodes\BlogSlider;
...
class BlogSlider implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		vc_map(
			array(
				'name'                      => esc_html__( 'Blog Slider', 'xtrail' ),
				'base'                      => $this->base,
				...
				'params'                    => array(
					array(
						...
						'value'       => array(
							esc_html__( 'Slider', 'xtrail' )            => 'slider',
							esc_html__( 'Carousel', 'xtrail' )          => 'carousel',
							esc_html__( 'Carousel Centered', 'xtrail' ) => 'carousel-centered'
						),
						'save_always' => true
					),
					array(
	...
	public function render( $atts, $content = null ) {
		...
		xtrail_select_get_module_template_part( 'shortcodes/blog-slider/holder', 'blog', '', $params );
    ...
    public function blogListCategoryAutocompleteSuggester( $query ) {
        ...
        if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
            foreach ( $post_meta_infos as $value ) {
                ...
                $data['label'] = ( ( strlen( $value['category_title'] ) > 0 ) ? esc_html__( 'Category', 'xtrail' ) . ': ' . $value['category_title'] : '' );
                $results[]     = $data;
            }
        }
