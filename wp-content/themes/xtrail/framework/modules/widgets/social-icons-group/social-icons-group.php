...
if ( class_exists( 'XtrailCoreClassWidget' ) ) {
	class XtrailSelectClassClassIconsGroupWidget extends XtrailCoreClassWidget {
		public function __construct() {
			parent::__construct(
				'qodef_social_icons_group_widget',
				esc_html__( 'Xtrail Social Icons Group Widget', 'xtrail' ),
				array( 'description' => esc_html__( 'Use this widget to add a group of up to 6 social icons to a widget area.', 'xtrail' ) )
			);
		...
		protected function setParams() {
			$this->params = array_merge(
				...
				xtrail_select_icon_collections()->getSocialIconWidgetMultipleParamsArray( 6 ),
				array(
					array(
						'type'    => 'dropdown',
						'name'    => 'layout',
						'title'   => esc_html__( 'Icons Layout', 'xtrail' ),
						'options' => array(
							''             => esc_html__( 'Default', 'xtrail' ),
							'square-icons' => esc_html__( 'Square', 'xtrail' ),
						)
					),
					array(
						'type'        => 'dropdown',
						'name'        => 'skin',
						'title'       => esc_html__( 'Square Icons Skin', 'xtrail' ),
						'description' => esc_html__( 'This applies to the square layout', 'xtrail' ),
						'options'     => array(
							''           => esc_html__( 'Dark Skin', 'xtrail' ),
							'light-skin' => esc_html__( 'Light Skin', 'xtrail' ),
						)
					),
					array(
						'type'    => 'dropdown',
						'name'    => 'alignment',
						'title'   => esc_html__( 'Text Alignment', 'xtrail' ),
						'options' => array(
					...
					array(
						'type'  => 'textfield',
						'name'  => 'icon_size',
						'title' => esc_html__( 'Icons Size (px)', 'xtrail' )
					),
					array(
						'type'  => 'colorpicker',
						'name'  => 'color',
		...
		public function widget( $args, $instance ) {
			...
			for ( $n = 1; $n <= 6; $n ++ ) {
				...
				<?php if ( ! empty( $icon_holder_html ) ) { ?>
					<a class="qodef-social-icon-widget-holder qodef-icon-has-hover" <?php echo xtrail_select_get_inline_attr( $hover_color, 'data-hover-color' ); ?> <?php xtrail_select_inline_style( $icon_styles ) ?> href="<?php echo esc_url( $link ); ?>" target="<?php echo esc_attr( $target ); ?>">
