...
$qodef_row_wrapper_start = $qodef_row_wrapper_end = $row_background_text = $row_bg_tex = $row_bg_text_size = $row_bg_text_color = $row_bg_text_size_1440 = $row_bg_text_size_1280 = '';
...
if(!empty($row_bg_text)) {

	$row_bg_text_style = array();

	if(!empty($row_bg_text_size)) {
		$row_bg_text_style[] = 'font-size:' . esc_html($row_bg_text_size);
	}
	if(!empty($row_bg_text_color)) {
		$row_bg_text_style[] = ' -webkit-text-stroke-color:' . esc_html($row_bg_text_color);
	}

	if(!empty($row_bg_text_align)) {
		$row_bg_text_style[] = 'text-align:' . esc_html($row_bg_text_align);
	}

	if(!empty($row_bg_text_vertical_align)) {
		$row_bg_text_style[] = 'vertical-align:' . esc_html($row_bg_text_vertical_align);
	}

	if(!empty($row_bg_text_vr_offset)) {
		$row_bg_text_style[] = 'top:' . esc_html($row_bg_text_vr_offset);
	}

	if(!empty($row_bg_text_hr_offset)) {
		$row_bg_text_style[] = 'left:' . esc_html($row_bg_text_hr_offset);
		$row_bg_text_style[] = 'width: calc(100% - ' . esc_html($row_bg_text_hr_offset) .')';
	}

	$row_bg_text_styles = implode(';', $row_bg_text_style);

	$row_bg_text_data = array();

	if(!empty($row_bg_text_size_1440)) {
		$row_bg_text_data[] = 'data-font-size-1440=' . esc_html($row_bg_text_size_1440);
	}

	if(!empty($row_bg_text_size_1280)) {
		$row_bg_text_data[] = 'data-font-size-1280=' . esc_html($row_bg_text_size_1280);
	}

    if(!empty($row_bg_text_vr_offset_1440)) {
        $row_bg_text_data[] = 'data-vr-offset-1440=' . esc_html($row_bg_text_vr_offset_1440);
    }

    if(!empty($row_bg_text_vr_offset_1280)) {
        $row_bg_text_data[] = 'data-vr-offset-1280=' . esc_html($row_bg_text_vr_offset_1280);
    }

    if(!empty($row_bg_text_hr_offset_1440)) {
        $row_bg_text_data[] = 'data-hr-offset-1440=' . esc_html($row_bg_text_hr_offset_1440);
    }

    if(!empty($row_bg_text_hr_offset_1280)) {
        $row_bg_text_data[] = 'data-hr-offset-1280=' . esc_html($row_bg_text_hr_offset_1280);
    }

	$row_bg_text_datas = implode(' ', $row_bg_text_data);

	$row_bg_text_wrapper_classes = '';

	if (!empty($row_bg_text_animation)) {
		if ($row_bg_text_animation == 'yes') {
			$row_bg_text_wrapper_classes .= 'qodef-row-bg-text-animation';
		}

		if (!empty($row_bg_text_align)) {
			$row_bg_text_wrapper_classes .= ' qodef-row-bg-text-align-'.$row_bg_text_align;
		}
	}

	$row_background_text  = '<div class="qodef-row-bg-text-holder">';
	$row_background_text .= '<div class="qodef-row-bg-text-wrapper '.esc_attr($row_bg_text_wrapper_classes).'">';
	$row_background_text .= '<div class="qodef-row-bg-text-wrapper-inner" ' . xtrail_select_get_inline_style($row_bg_text_styles) . ' ' . esc_html($row_bg_text_datas) . '>';

	$row_background_text .= '<div class="qodef-row-bg-text">';
	$row_background_text .= $row_bg_text;
...
if ( $row_content_width !== 'grid' ) {
	...
	if ( ! empty( $parallax_background_image ) ) {
		...
		$grid_row_data[]  = is_array($image_src) ? 'data-parallax-bg-image=' . esc_url( $image_src[0] ) : '';
...
if ( $row_content_width === 'grid' ) {
	$qodef_row_wrapper_start .= '<div class="qodef-row-grid-section-wrapper ' . esc_attr( $grid_row_classes ) . '" ' . esc_attr( $grid_row_datas ) . ' ' . xtrail_select_get_inline_style( $qodef_grid_row_styles ) . '><div class="qodef-row-grid-section">';
	$qodef_row_wrapper_end   .= '</div></div>';
}

$qodef_after_wrapper_open   = apply_filters('xtrail_select_filter_vc_after_wrapper_open', '', $atts);
$qodef_before_wrapper_close = apply_filters('xtrail_select_filter_vc_before_wrapper_close', '', $atts);
$css_classes                = apply_filters('xtrail_select_filter_vc_css_classes', $css_classes, $atts);
...
$output .= '<div ' . implode( ' ', $wrapper_attributes ) . ' ' . xtrail_select_get_inline_style( $qodef_vc_row_styles ) . '>';
...
echo xtrail_select_get_module_part( $output );
