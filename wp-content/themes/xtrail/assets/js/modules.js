...
(function($) {
    ...
    */
	...
	var qodefInitAnchor = function() {
		...
		var anchorClick = function () {
			qodef.document.on("click", ".qodef-main-menu a, .qodef-fullscreen-menu a, .qodef-btn, .qodef-anchor, .qodef-mobile-nav a, .qodef-vertical-menu a", function () {
				...
				if (hash !== '' && anchorData.length > 0) {
					var anchoredElementOffset = anchorData.offset().top,
						anchoredElementHeight = anchorData.outerHeight();
					
					if (qodef.windowWidth > 1366) {
						if (anchoredElementHeight > 600) {
							anchoredElementHeight = 0;
						}
					} else {
						anchoredElementHeight = 0;
					}

					scrollAmount = anchoredElementOffset - headerHeightToSubtract(anchoredElementOffset) - qodefGlobalVars.vars.qodefAddForAdminBar - anchoredElementHeight/2;
		...
		if (qodef.body.hasClass('qodef-smooth-page-transitions')) {
			...
			if (qodef.body.hasClass('qodef-smooth-page-transitions-preloader')) {
				...
				var fadeOutLoader = function(speed, delay, easing) {
					...
					loader.addClass('qodef-loader-loaded');
					if ($('.qodef-perspective-scroll').length > 0) {
						setTimeout(function() {
							$('.qodef-perspective-scroll').addClass('qodef-perspective-scroll-landing-loaded');
						}, 500);
					}
                                ...
                                <div class="pp_content"> \
                                    ...
                                    <div class="pp_fade"> \
                                        ...
                                        <div class="pp_details"> \
                                            <div class="pp_nav"> \
                                                <a href="#"class="pp_arrow_previous" rel="prev"><span class="arrow_carrot-left"></span></a> \
                                                <p class="currentTextHolder">0/0</p> \
                                                <a href="#" class="pp_arrow_next" rel="next"><span class="arrow_carrot-right"></span></a> \
                                            </div> \
                ...
                searchField.on('keyup paste', function() {
                    ...
                    keyPressTimeout = setTimeout( function() {
                        ...
                        if(searchTerm.length < 3) {
                            ...
                            var ajaxData = {
                                action: 'xtrail_select_search_post_types',
                                term: searchTerm,
                                postType: postType
                            };
	...
	/*
	 ** Init Linked Four Images
	 */
	function qodefInitFourImages(){
		var fourImagesHolder = $('.qodef-linked-expanding-images .qodef-linked-images-four-images');
		
		if(fourImagesHolder.length){
			fourImagesHolder.each(function() {
				var thisFourImagesHolder = $(this),
					items = thisFourImagesHolder.find('.qodef-linked-image-holder'),
					images = thisFourImagesHolder.find('.qodef-linked-image-image'),
					scaleVal;

				if (qodef.windowWidth > 1024) {
					thisFourImagesHolder.css({'display': 'flex', 'width': '100vw'});

					items.css({width:'25%', 'float': 'none', 'transition':'.5s'});
					
					if(qodef.windowWidth < 1367) {
						scaleVal = 1.8;
					} else {
						scaleVal = 1.3;
					}

					images.css({transform:'scale(' + scaleVal + ')'});
					images.css({'transition':'.5s', 'background-position': '13px 0'});

					thisFourImagesHolder.on('mouseleave', function() {
						items.css({width:'25%'});
					});
					
					items.on('mouseenter', function() {
						$(this).css({width:'29%'});
						$(this).find('.qodef-linked-image-image').css({'transform':'scale(' + scaleVal + ') translateX(20px)'});
				
					});

					items.on('mouseleave', function() {
						$(this).css({width:'25%'});
						$(this).find('.qodef-linked-image-image').css({transform:'scale(' + scaleVal + ')'});
					});
				}				
			});
		}
            ...
            sliders.each(function(){
	            ...
	            slider.waitForImages(function () {
		            owlSlider = slider.owlCarousel({
			            ...
			            nav: navigation,
			            navText: [
				            '<span class="qodef-prev-icon ' + qodefGlobalVars.vars.sliderNavPrevArrow + '"></span><span class="qodef-prev-dash"></span><span class="qodef-prev-text">' + qodefGlobalVars.vars.sliderNavPrevText + '</span>',
				            '<span class="qodef-next-text">' + qodefGlobalVars.vars.sliderNavNextText + '</span><span class="qodef-next-dash"></span><span class="qodef-next-icon ' + qodefGlobalVars.vars.sliderNavNextArrow + '"></span>'
			            ],
			            responsive: {
				            0: {
	...
	/**
     * Init Pattern Overlay animation
     */
    function qodefPatternOverlay() {
	    var patternOverlays = $('.qodef-pattern-overlay, .qodef-pattern-overlay-custom');
		
		if(patternOverlays.length){
			patternOverlays.each(function() {
				var thisPatternOverlay = $(this);

				thisPatternOverlay.appear(function () {
					$(this).addClass('qodef-pattern-appear');
				}, {
					accX: 0,
					accY: -400
				});
	...
	// Image With Text appear animation
	function qodefImageWithTextAnimation() {
		var imageWithText = $('.qodef-image-with-text-holder.qodef-image-behavior-custom-link');

		if (imageWithText.length) {
			imageWithText.css({'opacity': 0, 'transform': 'translateY(60px)'});
			...
			imageWithText.appear(function () {
				imageWithText.each(function () {
					$(this).css({
						...
						'transition': '1s 0.' + Math.floor(Math.random() * 4) + 3 + 's'
					});
				});
			}, {
				accX: 0,
    ...
    function qodefEventsListTitleHover() {

        var listPost = $('.tax-events_category article');
        ...
        if(listPost.length) {
            listPost.each( function() {
                var thisPost = $(this);
                var title = thisPost.find('.qodef-ttevents-single-title');
                var image = thisPost.find('.qodef-ttevents-single-image-holder');
	...
	function qodefRowBackgroundText() {
		var backgroundText = $('.qodef-row-bg-text-holder');

		backgroundText.each(function () {
			var backgroundTextInstance = $(this),
				backgroundTextWrapper = backgroundTextInstance.find('.qodef-row-bg-text-wrapper'),
				backgroundTextInner = backgroundTextInstance.find('.qodef-row-bg-text-wrapper-inner'),
				row = backgroundTextInstance.parent().addClass('qodef-row-has-bg-text'),
				backgroundTextCharsHolder = backgroundTextInstance.find('.qodef-row-bg-text');

			if(qodef.windowWidth > 1280 && qodef.windowWidth <= 1600) {
                ...
                if( typeof backgroundTextInner.data('vr-offset-1440') !== 'undefined' && backgroundTextInner.data('vr-offset-1440') !== false){
                    backgroundTextInner.css('top', backgroundTextInner.data('vr-offset-1440'));
                }
                if( typeof backgroundTextInner.data('hr-offset-1440') !== 'undefined' && backgroundTextInner.data('hr-offset-1440') !== false){
                    backgroundTextInner.css({
						'left' : backgroundTextInner.data('hr-offset-1440'),
						'width' : 'calc(100% - ' + backgroundTextInner.data('hr-offset-1440') +')'
                    }) ;
                }
			}
			...
			if(qodef.windowWidth > 1024 && qodef.windowWidth <= 1280){
                ...
                if( typeof backgroundTextInner.data('vr-offset-1280') !== 'undefined' && backgroundTextInner.data('vr-offset-1280') !== false){
                    backgroundTextInner.css('top', backgroundTextInner.data('vr-offset-1280'));
                }
                if( typeof backgroundTextInner.data('hr-offset-1280') !== 'undefined' && backgroundTextInner.data('hr-offset-1280') !== false){
                    backgroundTextInner.css({
                        'left' : backgroundTextInner.data('hr-offset-1280'),
                        'width' : 'calc(100% - ' + backgroundTextInner.data('hr-offset-1280') +')'
                    }) ;
                }
			}
			...
			//appear fx
			if (backgroundTextWrapper.hasClass('qodef-row-bg-text-animation') && !qodef.htmlEl.hasClass('touch')) {
				var bgrndTextElements = backgroundTextInstance.find('.qodef-row-bg-text'),
					delay = 150;
				...
				bgrndTextElements.each(function(i){
					...
					var firstTextChars = bgrndTextElements.filter('.qodef-row-bg-text').find('.qodef-char-mask').length;
					...
					if (backgroundTextInstance.parent().hasClass('qodef-wait-for-preloader')) {
						...
						$(window).on('load', function(){
							backgroundTextCharsHolder.appear(function(){
								showText();
							}, {accX: 0, accY: 100});
						});
					}
...
(function($) {
    ...
    */
	...
	function qodefInitBlogPagination(){
		...
		var initMainPagFunctionality = function(thisHolder) {
			...
			if(nextPage <= maxNumPages){
				...
				var ajaxData = qodef.modules.common.setLoadMoreAjaxData(loadMoreDatta, 'xtrail_select_blog_load_more');
	...
	function qodefTitleHover() {
		var holder = $('.qodef-blog-holder');
		var post = holder.find('article');
		var listPost = $('.qodef-bl-item');
		var relatedPost = $('.qodef-related-post');
		...
		if(post.length) {
			post.each( function() {
				...
				if (thisPost.hasClass('format-standard') && !holder.hasClass('qodef-blog-single')) {
					var title = thisPost.find('.qodef-post-title a');
					var imageholder = thisPost.find('.qodef-post-heading');
					var image = imageholder.find('.qodef-post-image');
					qodefTitleHoverClass(title, image);
				}
			});
		}
        ...
        if(listPost.length) {
            listPost.each( function() {
                ...
                if (! thisPost.hasClass('format-quote', 'format-link', 'format-audio')) {
                    var title = thisPost.find('.qodef-post-title a');
                    var image = thisPost.find('.qodef-bli-heading');
        ...
        if(relatedPost.length) {
            relatedPost.each( function() {
                var thisPost = $(this);
				var title = thisPost.find('.qodef-post-title a');
				var image = thisPost.find('.qodef-related-post-image');
    ...
    function qodefInitBlogListShortcodePagination(){
        ...
        var initMainPagFunctionality = function(thisHolder, pagedLink) {
            ...
            if(nextPage <= maxNumPages){
                ...
                var ajaxData = qodef.modules.common.setLoadMoreAjaxData(loadMoreDatta, 'xtrail_select_blog_shortcode_load_more');
                ...
                searchOpener.each(function () {
                    ...
                    thisOpener.on('click', function (e) {
                        ...
                        if (searchForm.hasClass('qodef-is-active')) {
                            searchForm.height(searchFormHeight).stop(true).fadeIn(300).find('input[type="text"]').focus();
                        }

                        searchForm.find('.qodef-search-close').on('click', function (e) {
                            e.preventDefault();
                            searchForm.stop(true).fadeOut(300, function () {
